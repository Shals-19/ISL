...
class ISLTranslator(nn.Module):
    """Real-time ISL to Text translator with Dense Vision Transformer.
    
    Architecture:
        VideoEncoder (Dense ViT + Temporal Convolutions + Dense Attention)
            ↓
        TextDecoder (Transformer)
    
    Dense Configuration:
    - Vision Transformer: 24 layers, 1024 hidden dim, 16 heads
    - Temporal Convolutions: 4 blocks for motion modeling
    - Attention Pooling: Dense with refinement layers
    - Total ~100M parameters (enables overfitting on 6000+ videos)
    ...
    Training:
        ...
        - No backbone freezing (all layers trainable from start)
